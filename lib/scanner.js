...
async function scanSite(rawUrl) {
  ...
  // Te crawlen pagina's bepalen (PDF's en andere binaire bestanden overslaan)
  let queue = [base + '/'];
  if (siteChecks.sitemapUrls && siteChecks.sitemapUrls.length) {
    queue = [base + '/', ...siteChecks.sitemapUrls.filter(u => sameHost(base, u))];
  }
  queue = [...new Set(queue.map(u => cleanUrl(base, u)).filter(u => u && !isBinair(u)))].slice(0, MAX_PAGES);
    ...
    try {
      ...
      if (!siteChecks.sitemap && i === 0) {
        ...
        $('a[href]').each((_, el) => {
          ...
          if (u && sameHost(base, u) && !isBinair(u) && !gezien.has(u) && queue.length < MAX_PAGES) {
