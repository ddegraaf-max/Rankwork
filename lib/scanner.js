...
async function scanSite(rawUrl) {
  ...
  const telDubbel = (waardes) => {
    const m = new Map();
    for (const w of waardes) { const k = (w || '').trim().toLowerCase(); if (k) m.set(k, (m.get(k) || 0) + 1); }
    let n = 0; for (const aantal of m.values()) if (aantal > 1) n += aantal;
    return n;
  };
  const vergelijkbaar = pages.filter(p => !p.geblokkeerd);
  siteChecks.dubbeleTitles = telDubbel(vergelijkbaar.map(p => p.title));
  siteChecks.dubbeleDescriptions = telDubbel(vergelijkbaar.map(p => p.desc));
