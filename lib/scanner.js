...
async function checkSiteLevel(base) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (lijktRobots(txt)) {
        out.robots = true;
        out.robotsBlokkeertAlles = /^disallow:\s*\/\s*$/mi.test(txt) && /^user-agent:\s*\*/mi.test(txt);
...
function genereerTaken(result) {
  ...
  if (result.siteChecks.robotsBlokkeertAlles) push('robots.txt blokkeert de hele site!',
    'In robots.txt staat "Disallow: /" voor alle bots — Google indexeert dan niets. Verwijder die regel.', 'hoog');
