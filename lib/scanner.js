...
// Parseert robots.txt in user-agent-blokken en beoordeelt of GOOGLE (of *)
// daadwerkelijk volledig geblokkeerd wordt. Een "Disallow: /" die alleen voor
// AI-bots geldt (GPTBot, ClaudeBot, enz. — bijv. Cloudflare's managed robots.txt)
// is GEEN blokkade voor de zoekindexering.
...
function genereerTaken(result) {
  ...
  if (!result.siteChecks.sitemap) push('Voeg een sitemap.xml toe',
    ...
    + (result.siteChecks.catchAll ? ' Let op: je server geeft voor elk pad een 200 terug; zorg dat /sitemap.xml échte XML serveert.' : ''), 'hoog');
  if (!result.siteChecks.robots) push('Voeg robots.txt toe',
    'Er is geen (geldige) robots.txt gevonden. Maak er een met een verwijzing naar je sitemap (Sitemap: https://…/sitemap.xml).'
    + (result.siteChecks.catchAll ? ' Let op: je server geeft voor elk pad een 200 terug; zorg dat /robots.txt échte tekst serveert.' : ''), 'middel');
  if (result.siteChecks.robotsBlokkeertAlles) push('robots.txt blokkeert Google!',
    'Het blok dat voor Googlebot/* geldt bevat "Disallow: /" — Google indexeert dan niets. Let op: "Disallow: /" dat alleen voor AI-bots geldt (GPTBot, ClaudeBot e.d.) is géén probleem en triggert deze melding niet.', 'hoog');
  if (result.pages.some(p => p.geblokkeerd)) push('Botbescherming blokkeert de scanner',
    'Een of meer pagina\'s tonen een challenge-pagina aan RankWerkBot. Whitelist de bot of het server-IP; tot die tijd zijn de scanresultaten onvolledig.', 'middel');

  // Meest voorkomende paginaproblemen bundelen (geblokkeerde pagina's overslaan)
  const teller = {};
  for (const p of result.pages) {
    if (p.geblokkeerd) continue;
