...
const { scanSite, genereerTaken } = require('./scanner');
...
async function scanRonde() {
  ...
  try {
    ...
    try {
      ...
      console.log(`🔄 Automatische scan ${site.naam}: score ${result.score}`);
    } catch (e) {
  ...
  const rij = r => `
    <tr>
      ...
      <td style="padding:8px;border-bottom:1px solid #eee;text-align:center;font-size:20px;font-weight:bold;
          color:${r.score >= 80 ? '#16a34a' : r.score >= 55 ? '#d97706' : '#dc2626'}">
